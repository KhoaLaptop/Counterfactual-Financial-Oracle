...
from src.models import ScenarioParams
...
with st.sidebar:
    ...
    # PDF Upload
    st.markdown('<p class="metric-label">Input Document</p>', unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        for field, value in sources.items():
            with st.expander(f"**{field}**: {value}"):
                source_info = report.index.get(field, "Extracted from Landing AI ADE")
                st.write(f"📍 **Source**: {source_info}")
                
                # Show original text if available in notes
                if field in report.notes:
                    st.write(f"📝 **Note**: {report.notes[field]}")
                
                # Show metadata if available
                if report.source_metadata:
                    matching = [m for m in report.source_metadata if m.field_name == field]
                    if matching:
                        meta = matching[0]
                        if meta.source_page:
                            st.write(f"📄 **Page**: {meta.source_page}")
                        if meta.source_text:
                            st.write(f"💬 **Original Text**: _{meta.source_text}_")
    ...
    if st.session_state.simulation_results is not None:
        ...
        if st.session_state.debate_result is not None:
            ...
            with col1:
                ...
                if debate.key_agreements:
                    for agreement in debate.key_agreements:
                        st.markdown(f"- {agreement}")
                else:
                    st.markdown("- No explicit agreements recorded")
            
            with col2:
                st.markdown("**⚠️ Remaining Concerns:**")
                if debate.key_disagreements:
                    for disagreement in debate.key_disagreements:
                        st.markdown(f"- {disagreement}")
                else:
                    st.markdown("- No major disagreements")
